...
\usepackage[paper=a4paper,left=2cm,top=2cm,right=2cm,bottom=2cm]{geometry}%
...
% Dates
\date{\today} % when you are in the process of writing
...
modelDesigns <- c("block (R)",
                  ...
                  "replicate (F)",
                  "row (R) + col (R)",
                  "row(replicate) (R) + col(replicate) (R)")
...
descrItems <- c(x$traits, modelDesigns[x$design],
                if (engine == "SpATS") "splines" else "no",
                modelType)
names(descrItems) <- c("response",
                       "design features",
                       "spatial model",
                       "genotype")
...
print(xtable::xtable(x = modSum[, c(1, 3)], caption = "Data summary",
                     label = "modelSummary",
                     align = c("l", "r", "c")),
