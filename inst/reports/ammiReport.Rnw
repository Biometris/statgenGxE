...
%-------------------------------------------------------------------------------
% Title
\title{AMMI report for \Sexpr{escapeLatex(x$trait)}}%
% Authors
\author{\vspace{-5ex}}
%-------------------------------------------------------------------------------
% Dates
...
if (x$byYear) {
  years <- names(x$anova)
  for (year in years) {
    printAnova(aovTab = x$anova[[year]], title = year)
  }
} else {
  printAnova(aovTab = x$anova)
}
@
\clearpage
%-------------------------------------------------------------------------------
...
if (x$byYear) {
  for (year in years) {
    importance <- x$importance[[year]][, 1:ncol(x$envScores[[year]])]
    print(xtable::xtable(x = importance, capion = year,
  ...
  importance <- x$importance[, 1:ncol(x$envScores)]
...
\section{Biplots}
%-------------------------------------------------------------------------------
<<label=biplot, echo=FALSE, fig.width=9, fig.height=6.5, out.width="0.9\\maxwidth", background="white">>=
plot(x, plotType = "AMMI2")
@
