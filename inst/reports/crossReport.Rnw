...
summTab <- data.frame(c("Cross type", NA, "Number of individuals",
                      ...
                      c(switch(summ$type, "f2" = "F2 intercross",
                        ...
                        NA,
                        summ$n.phe,
                        ## If there is an X chr misssing.phe is a vector so
                        ## output needs pasting together.
                        paste(round((1 - summ$missing.phe) * 100, 1),
                              collapse = " "),

                        NA,
                        summ$n.chr,
                        if (!is.null(summ$autosomes)) paste(summ$autosomes,
                                                            collapse = " "),
                        if (!is.null(summ$Xchr)) summ$Xchr,
                        NA,
                        sum(summ$n.mar),
                        ...
                        if (!is.null(summ$typing.freq.X)) {
                          ...
                          genoX <- paste(names(summ$typing.freq.X), roundedX,
                                         sep = ":")
                          if (!is.null(summ$typing.freq.A)) {
                            roundedA <- sprintf("%.1f", summ$typing.freq.A * 100)
                            genoA <- paste(names(summ$typing.freq.A), roundedA,
                                           sep = ":")
                          ...
                          geno <- paste(names(summ$typing.freq),
                                        sprintf("%.1f", summ$typing.freq * 100),
                                        sep = ":")
                          paste(geno, collapse = " ")
                        }))
...
print(xtable::xtable(x = summTab, label = "summary",
                     align = c("l", "l", "X")),
      latex.environments = "flushleft", tabular.environment = "tabularx",
...
%-------------------------------------------------------------------------------
<<label=segrDist, echo=FALSE, results="asis">>=
...
if (nrow(segr) > 15) {
  ## Too many markers to print. Exported to .csv instead.
  cat(paste(nrow(segr), "markers show evidence of segregation distortion."))
  cat(paste0("\\newline Those markers are exported to ",
             file.path(outDir, paste0(outBase, "csv"))))
  ## Writing directly to outDir doesn't work when relative paths are used.
  ## Relative path while knitting is different from actual relative path.
  ## Therefore .csv is written to package directory and moved from there.
  write.csv(x = segr, file = paste0(outBase, "csv"))
} else if (nrow(segr) > 0) {
...
## checkAlleles doesn't work for all types of cross.
if (summ$type %in% c("f2", "bc", "risib", "riself", "dh", "haploid")) {
  qtl::checkAlleles(crossRec)
}
@
<<label=recomPlot, echo=FALSE, out.width='85%'>>=
## Plot of recombination frequencies
qtl::plotRF(crossRec, alternate.chrid = TRUE)
@
\newpage
