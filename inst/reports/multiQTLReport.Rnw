...
names(bgItems) <- c("Modeling date", "Modeling time",
                    "Pipeline version")
...
modSum <- data.frame(c("Full model", "Selection", "Final model"),
                     c(paste(xtable::sanitize(x$QTLDet$trait), "\\texttildelow\\",
                       xtable::sanitize(paste(rownames(x$QTLDet$peaks),
                                              collapse = " + "))),
                     x$selection,
                     paste(xtable::sanitize(x$QTLDet$trait), "\\texttildelow\\",
                       xtable::sanitize(paste(rownames(x$qtl$result.drop),
                                              collapse = " + ")))))
...
print(xtable::xtable(x = fitted, caption = "List of fitted markers",
                     label = "fitted",
                     align = c("l", "r", "r")),
...
effectTab <- setNames(as.data.frame(x$qtl$ests$ests), "estimate")
## Add significance stars.
