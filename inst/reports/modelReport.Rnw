...
%-------------------------------------------------------------------------------
<<label=background, echo=FALSE, results="asis">>=
bgItems <- c(format(attr(x, "timestamp"), "%y-%m-%d"),
             format(attr(x, "timestamp"), "%H:%M:%S"),
             x$engine, as.character(packageVersion("RAP")))
names(bgItems) <- c("Modeling date", "Modeling time", "Modeling engine",
                    "Pipeline version")
print(xtable::xtable(x = data.frame(bgItems), label = "background",
                     align = c("l", "l")),
...
descrItems <- c(x$traits, modelDesigns[x$design],
                if (is.null(x$spatial)) "id" else as.character(x$spatial[1]),
                modelType)
...
renameFrom <- c("genotype", "repId", "rowId", "colId", "subBlock",
                ...
                "f(colCoordinates):f(rowCoordinates)", "Nobs", "R", "variance",
                "pow", "units")
...
if (engine == "SpATS") {
  ...
  varComp <- summary(model)$varcomp[, "component", drop = FALSE]
  ...
  if ("Genotype" %in% rownames(varComp)) {
    varComp <- rbind(varComp["Genotype", , drop = FALSE],
                     varComp[-which(rownames(varComp) == "Genotype"), , drop = FALSE])
  }
...
%-------------------------------------------------------------------------------
<<label=spatialPlots, echo=FALSE, results="asis">>=
if (all(c("colId", "rowId") %in% colnames(x$data))) {
...
%-------------------------------------------------------------------------------
\section{List of best 10\% genotypes with highest \Sexpr{predName}}
%-------------------------------------------------------------------------------
<<label=best20, echo=FALSE, results="hide">>=
pred <- STExtract(x, what = predName)
predSE <- STExtract(x, what = paste0("se", predName))
predTot <- merge(pred, predSE , by = "genotype")
predTot <- predTot[order(predTot[[paste0(x$traits,".x")]], decreasing = TRUE), ]
colnames(predTot) <- c("genotype", x$traits, "SE")
...
@
\columnbreak
<<label=predPlot, echo=FALSE>>=
## Plot of predictions, BLUEs or BLUPs
plot(rev(predTot[[x$traits]]), col = "#0080ff", xlab = "Genotypes", xaxt = "n",
     cex.lab = 1.5, ylab = x$traits, mar = c(5, 5, 4, 2) + 0.1)
abline(h = quantile(x = predTot[[x$traits]], probs = 0.9), col = "red")
abline(h = quantile(x = predTot[[x$traits]], probs = 0.1), col = "red")
@
\end{multicols*}
%-------------------------------------------------------------------------------
% End Document
