...
renameFrom <- c("genotype", "repId", "rowId", "colId", "subBlock",
                ...
                "f(colCoordinates):f(rowCoordinates)", "Nobs", "R")
renameTo <- c("Genotype", "Replicate", "Row", "Col", "Block",
              ...
              "Number of observations", "Residual")
if (engine == "SpATS") {
...
pred <- STExtract(x, what = if (modelType == "fixed") "BLUEs" else "BLUPs")
pred <- pred[order(pred[[x$traits]]), x$traits]
best <- summary(x, nBest = ceiling(length(pred) / 10))
@
\begin{multicols*}{2}
...
plot(pred, col = "#0080ff", xlab = "Genotypes", xaxt = "n", cex.lab = 1.5,
     ylab = if (modelType == "fixed") "BLUEs" else "BLUPs",
     mar = c(5, 5, 4, 2) + 0.1)
abline(h = quantile(x = pred, probs = 0.9), col = "red")
@
\end{multicols*}
