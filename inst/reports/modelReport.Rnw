...
%-------------------------------------------------------------------------------
% Title
\title{Single trial analysis for \Sexpr{escapeLatex(trait)}}%
% Authors
\author{\vspace{-5ex}}
%-------------------------------------------------------------------------------
% Dates
...
## Create summary SpATS in separate chunck to suppress output.
if (engine == "SpATS") {
...
varComp <- STExtract(x, trials = trial,
...
if (all(c("colId", "rowId") %in% colnames(x[[trial]]$TD[[trial]]))) {
  plot(x, trial = trial, what = modelType, plotType = "spatial", outCols = 2)
}
@
...
pred <- STExtract(x, what = predName)[[trial]][[predName]]
predSE <- STExtract(x, what = paste0("se", predName))[[trial]][[1]]
