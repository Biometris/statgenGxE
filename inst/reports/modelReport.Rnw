...
descrItems <- c(trait, modelDesigns[x$design],
...
modSum <- as.data.frame(summary(x$data, traits = trait))
...
predTot <- STExtract(x, what = predName) %>%
...
abline(h = quantile(x = predTot[[trait]], probs = 0.9), col = "red")
abline(h = quantile(x = predTot[[trait]], probs = 0.1), col = "red")
@
\end{multicols*}
%-------------------------------------------------------------------------------
% End Document
