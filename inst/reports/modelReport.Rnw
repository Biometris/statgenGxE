...
print(xtable::xtable(x = modSum,
                     caption = "Size of the dataset and eight number descriptives",
                     label = "modelSummary", align = c("l", "r")),
      latex.environments = "flushleft", caption.placement = "top",
      include.rownames = TRUE, include.colnames = FALSE)
@
...
if (engine == "SpATS") {
  ...
  for (i in seq_along(renameVars[["renameFrom"]])) {
    rownames(effDims)[rownames(effDims) == renameVars[["renameFrom"]][i]] <-
...
if (modelType == "random") {
  cat(paste("The generalized heritability is",
            round(extract(x, trials = trial,
                            what = "heritability")[[trial]][["heritability"]],
                  digits = 2)))
}
@
\clearpage
