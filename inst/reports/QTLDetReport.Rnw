...
modSum <- data.frame(c("QTL Detection model", "Method", "Step size",
                       "Window"),
                     c(switch(x$type, "MR" = "Marker response",
                               "SIM" = "Simple interval mapping",
                               "CIM" = "Composite interval mapping"),
                       "Haley Knot", x$info$step, x$info$window))
...
summaryTab <- x$peaks
print(xtable::xtable(x = summaryTab, caption = "List of candidate QTLs (peaks)",
                     label = "summary",
                     align = c("l", "r", "r", "r", "r"),
                     digits = c(0, 0, 0, 0, 2)),
