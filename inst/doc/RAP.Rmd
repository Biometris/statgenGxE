...
Alternatively, spatial models can be fitted using asreml as modeling engine. In
this case 6 models are fitted and the best model, based on a goodness-of-fit 
criterion, either AIC or BIC, is chosen. For a full specification of the models 
fitted see `help(STRunModel)`. The criterion to be used can be specified using
the `control` parameter in `STRunModel`.  
...
`modWheatSp` it is possible to calculate both BLUEs and BLUPs. Therefore both 
will show in the summary of the model together with their respective standard 
errors.
