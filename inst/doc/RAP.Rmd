...
* Modeling and extracting results
...
the RAP package is always creating an object of class `TD`. Such an object
is used by the RAP package for performing most analyses.  
...
None of these is strictly neccessary for any analysis but the meta data is 
...
first create a `TD` object without adding meta data. This will still add a 
...
For the example first an `TD` object is created for the first location and 
...
later on. A trade off is that fitting 2 models takes more time than fitting only
1 so when fitting models on large datasets it is sensible to use `what` if only 
some results are needed as output.

## Model output

Since genotype has been fitted both as fixed and as random factor in `modWheat`
it is possible to calculate both BLUEs and BLUPs. Therefore both will show in 
the summary of the model together with their respective standard errors.
...
As with many object in the RAP package there is a `report` function available 
for `SSA` objects. This `report` function will create a pdf report describing 
the main results of the fitted model. Also a tex file will be created that was 
...
file. Non-existing directories are be created by the `report` function.  

The report contains some background information on the model fitted, a 
...
trait and 1 trial or a `list` of different results for all models in an `SSA` 
object. The results that can be extracted depend on the type of model fitted and 
sometimes on the modeling engine as well. For example, BLUEs can only extracted 
if genotype was fitted as fixed effect whereas BLUPs and heritabilities can only
be extracted and calculated if genotype was fitted as random effect.  
...
Below are some examples of extracting results from a fitted model. Recall that `modWheat` contains 2 fitted models, 1 with genotype as fixed effect and 1 with genotype as random effect.
```{r extBLUEs}
## Extract BLUEs
...
items, the `data.frames` BLUEs and BLUPs. 
