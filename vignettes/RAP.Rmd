...
`dropTD` specifying the trials to drop in the parameter `rmTrials`.
...
was performed in 2 locations in Chili, Santa Rosa and Cauquenes, with 2 
different drought regimes in 2011 and 2012 for the Santa Rosa and 1 trial 
in 2012 for Cauquenes. For 384 genotypes 4 traits were measured in 2011 but in 
...
a factor. The columns "rep" and "bl" have been renamed and converted likewise.
The columns "row" and "col" have been renamed to "rowCoord" and "colCoord" 
respectively. Simultaneously two columns "rowId" and "colId" have been 
...
"colCoord". The information about which columns have been renamed in the 
process of creating a `TD` object is stored as ab attribute of each individual
...
be used. Since now only one new trial is added, it makes sense to immediately dd the 
...
based on trLat and trLong in the meta data. If longitude or latitude is not available 
...
for the (groups of) trials can be ordered using `orderBy`.
```{r boxPlotGR}
...
Models can be fitted for 5 different trial designs. These are listed in the 
...
`what` if only a subset of the results are needed as output.
...
```
The fitted models and the best model are stored in the output as well as a summary
...
`modWheatSp` it is possible to calculate both the Best Lineair Unbiased Estimators 
(BLUEs) and the Best Lineair Unbiased Predictors (BLUPs). Therefore both are printed 
...
Two types of plots can be made based for the fitted models. The first is a series
...
`plot` on the `SSA` object. Plot can be made for multiple trials and multiple traits 
...
extra parameter `plotType = "spatial"`. Note that spatial plots can only be made spatial information, `rowCoord` and `colCoord`, is available in the `TD` object.
```{r spatPlot}
...
the main results of the fitted model. Also the tex file and images used for generating
...
When no outfile is specified a report will be created with a default name, i.e. "modelReport", in the current working directory. The parameter `outfile` can be used to change the location. This value of this parameter should be a valid location and name for a **pdf** file, i.e. including the postfix ".pdf". Non-existing directories are created by the `report` function.  
...
converted again into an `TD` object. For this the function `SSAtoTD` can be used. It
creates a `TD` object from a fitted model outputting one or more of the following:
BLUEs, standard errors of BLUEs, BLUPs and standard errors of BLUPs. Optionally a column `wt` with weights (calculated as $(1 / standard error of BLUEs)^2$) can be added as well. In the same way as described in the previous section extra columns can be added to the output using the parameter `keep`. 
```{r SSAtoTD}
