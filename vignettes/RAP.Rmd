...
and setting the parameter `trySpatial = TRUE` . In this case 7 models are fitted 
...
```
The fitted models and the best model are stored in the output:
```{r spatCh}
if (requireNamespace("asreml", quietly = TRUE)) {
  ## Best model
...
which results were extracted. These items are a `list` themselves with an item for every statistic that has been extracted, so 1 item for `BLUEsWheat`, a `data.frame` containing the BLUEs and 2 for `predWheat`, the `data.frames` containing BLUEs and BLUPs respectively. 
...
To use the BLUEs or BLUPs from the fitted model in a GxE analysis they have to be converted again into an `TD` object. For the the function `SSAtoTD` can be used. It
creates a `TD` object from a fitted model outputting one or more of the following:
BLUEs, standard error of BLUEs, BLUPs, standard error of BLUPs. Optionally a column `wt` with weights (1 / standard error of BLUEs) can be added as well. In the same way as described in the previous section extra columns can be added to the output using the parameter `keep`. 
```{r SSAtoTD}
