...
* [Mixed model analysis of two-way GxE tables](#vcomp)
* [Finlay-Wilkinson Analysis](#fw)
* [AMMI Analysis](#am)
* [GGE Analysis](#gge)
* [Identification of mega environments](#me)
* [Computation of stability measures](#st)
* [Modelling of heterogeneity of genetic variances and correlations](#vcov)
...
The use of the package is demonstrated using maize data from the European Union project DROPS. The data is available from https://datae.inra.fr/dataset.xhtml?persistentId=doi:10.15454/IASSTN [@Millet2019] and the relevant data set is included as data.frame in the statgenGxE package.
...
For further insight a scatter plot of the trials against each other can be made. 

```{r TDscatter, fig.dim = c(9, 9)} 
...
To get a first idea of the structure of the genotype by environment data, a mixed model can be fitted. In the statgenGxE package this can be done using the `gxeVarComp` function. Based on the structure of the genotype by environment table of observations and the chosen parameters this function will fit a model best fitting to the data.    

Six different types of models can be fitted depending on the environmental dimensions of the data. These models are described in the table below, together with the parameters used in `gxeVarComp` to fit the model.

| environmental dimensions | model | function parameters |
|:-----------|:-------------------|:------------|
| environments correspond to trials | **trait** = trial + **genotype + genotype:trial** | | trials form a factorial structure of locations x years | **trait** = year + location + year:location + **genotype + genotype:year + genotype:location + genotype:year:location** | `locationYear = TRUE` |
...
| trials correspond to locations within regions across years | **trait** = region + region:location + year + region:year + region:location:year + **genotype + genotype:region:location + genotype:year + genotype:region:year + genotype:region:location:year** | `regionLocationYear = TRUE` |
...
*Note that if the data contains replicates or weights are applied (using the option `useWt`) the last model term will be dropped, and estimated as a homogeneous residual.*    
 
All models can be fitted using either `lme4` or `asreml`.  

For diagnostics purposes and to identify the main sources of variation, all models are fitted three times:      
...
Then a model is fitted where all model terms are included as random terms. Based on the variance components in this model the percentage of variance explained by each of the model components is determined. This is printed in the model summary and plotted as standard deviations of the estimate.    

### Mixed model {-}
Finally a mixed model is fitted as specified in the table above. Based on these model variance components can be extracted, heritabilities can be computed and predictions can be made. It is also possible to plot the results.
...
```
The diagnostics for the fitted model can be printed using the `diagnostics` function. This will print a table of missing values for each of the terms in the random part of the fitted model.
...
Extracting the variance components from the fitted model can be done using the `vc` function. The heritability is computed using `herit`.
...
A plot can be made of the square roots of the variance estimates. These are based on the fully random model.
```{r VarCompPlot}
## Plot the results of the fitted model.
plot(dropsVarComp)
```
Predictions can be made for model terms that represent different levels of data aggregation; for the genotype main effect or for genotypic performance in grouped environments, or for genotypic performance in individual trials. These levels can be specified by the parameter `predictLevel`.
...
The model fitted in the analysis is $y_{ij} = \mu + G_i + \beta_iE_j + \epsilon_{ij}$, where $y_{ij}$ is the phenotypic value of genotype $i$ in environment $j$, $\mu$ is the general mean, $G_i$ is the genotypic mean, $\beta_i$ the sensitivity parameters, $E_j$ the environment effects and $\epsilon_{ij}$ the error.
...
summary(dropsFW)
```
Three types of plots can be made to investigate the output from the analysis. `plotType = "scatter"` creates three scatter plots where genotypic mean, mean squared deviation and sensitivity are plotted against each other. 
...
With `plotType = "line"` a plot with slopes for all genotypes in the analysis is created. 
...
The model fitted in the analysis is $y_{ij} = \mu + G_i + E_j + \Sigma_{m =1}^M\delta_{mi}\gamma_{mj} + \epsilon_{ij}$, where $M$ is the number of principal components, $y_{ij}$ is the phenotypic value of genotype $i$ in environment $j$, $\mu$ is the general mean, $G_i$ is the genotypic mean, $E_j$ the environmental mean, $\delta_{mi}$ the environmental scores, $\gamma_{mj}$ the genotypic scores and $\epsilon_{ij}$ the error.

The AMMI analysis can be performed with the statgenGxE package using the function `gxeAmmi`. By default the residuals are not scaled when performing the principal component analysis. Scaling them can be done by setting `scale = TRUE` when calling `gxeAmmi`.
```{r geAmmi}
...
The results of an AMMI analysis can be displayed in a two types of plot are available. "AMMI1" plots the first principal component against the main effects. "AMMI2" plots the second against the first principal component. 
```{r plotAmmi1, fig.width=5, fig.height=5, out.width="75%"}
## Create an AMMI1 plot.
plot(dropsAm, plotType = "AMMI1")
```

In AMMI2 biplots the parameter `scale`, a value between 0 and 1, can be used for changing the ratio between environmental scores and genotypic scores. A scale value of 0 stands for genotype scaling, i.e. all weight on the genotypic scores and the environments contracted towards the center. A scale value of 1 stands for environment scaling, i.e. all weight on the environment scores and the genotypes contracted towards the center. Symmetric scaling, using a scale value of 0.5, applies equal weights to environment and genotypic scores.  
...
A convex hull can be plotted around the genotypes in an AMMI2 biplot with lines from the origin perpendicular to the edges of the hull. 
...
```{r geMegaEnv, R.options=list(digits=3)}
...
```{r geMegaEnvPred, R.options=list(digits=3)}
...
To compute the stability measures use `gxeStability`. This will compute all three stability measures described above. This can be restricted by specifying one or more measure in `method`.

```{r geStab, R.options=list(digits=3)}
...
```{r geStabMegaEnv, R.options=list(digits=3), eval = FALSE}
...
The function `gxeVarCov` allows to fit linear mixed models of diverse complexity. Models can be fitted using either lme4 or asreml as modeling engine. If the engine is set to 'lme4', only a model considering multiple variance components, but with a compound symmetry variance-covariance structure can be fitted. If the engine is set to 'asreml', complex variance-covariance structures will be fitted sequentially to the genotype by environment data. The best fitting model is then identified.    
The models fitted are of the form $y_{ij} = \mu_i + \epsilon_{ij}$, where $y_{ij}$ is the phenotypic value of genotype $i$ in environment $j$, $\mu_i$ is the genotypic mean and $\epsilon_{ij}$ the residual genetic variation. This residual genetic variation is modeled in eight ways as described in the table below (see [@Boer2007]).
...
In this table $J$ is the number of environments, $\sigma_G^2$ the variance component for the genotype main effects, $\sigma_{GE}^2$ the variance component for GxE interactions. $\sigma_{G_j}^2$ and $\sigma_{GE_j}^2$ are the environment specific variance components for the genotype main effects and GxE interaction in environment $j$. $\sigma_{G_{j,k}}^2$ is the genetic covariance between environments $j$ and $k$. $\theta$ is the common correlation between environments and $\lambda_{1j}$ and $\lambda_{2j}$ are environment specific multiplicative
parameters.
...
```
`r if (exists("dropsVC2")) {"As becomes clear from the summary, the best model based on AIC is the model with an unstructured variance-covariance structure. This indicates the presence of GxE in the data."  }` 
