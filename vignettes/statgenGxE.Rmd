...
* [Select the best variance-covariance model for a set of trials](#vcov)
...
The model fitted in the analysis is $y_{ij} = \mu + G_i + E_j + \delta_i\gamma_j + \epsilon_{ij}$, where $y_{ij}$ is the phenotypic value of genotype $i$ in environment $j$, $\mu$ is the general mean, $G_i$ is the genotypic mean, $E_j$ the environmental mean, $\delta_i$ the environmental scores, $\gamma_j$ the genotypic scores and $\epsilon_{ij}$ the error.
...
The results of an AMMI analysis can be displayed in a biplot. Two types of biplot are available. "AMMI1" plots the first principal component against the main effects. "AMMI2" plots the second against the first principal component. 
...
The model fitted in the GGE analysis is $y_{ij} = \mu + E_j + \delta_i\gamma_j + \epsilon_{ij}$, where the parameters are as described in the AMMI analysis.
