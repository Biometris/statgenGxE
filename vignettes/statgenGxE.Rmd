...
*Note that if the data contains replicates or weights are applied (using the option `useWt`) the last term for each model will be dropped, and replaced by a homogeneous residual. Multiple observations will then be treated as in in completely randomized design.*    
...
### Mixed model {-}
Finally a mixed model is fitted as specified in the table above. Based on this model variance components can be extracted, heritabilities on a line means basis can be computed and predictions can be made. It is also possible to plot the results.
...
In the statgenGxE package this analysis can be done using the `gxeFW` function. The model describe above is fitted using an alternating regression algorithm. First, using starting values for $beta$ and $G$, $E$ is estimated. Next, $E$ is assumed known and $beta$ and $G$ are estimated. This process is continued until convergence, i.e. untils the change in $beta$ between iterations is less then a specified tolerance (default 0.001). When estimating values missing values are estimated as wel.  
