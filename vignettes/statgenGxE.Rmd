...
* [Mixed model analysis of two-way GxE tables](#vc)
...
```
the axes specifying `primAxis` and `secAxis`.  
Genotypes can be grouped and colored by a variable in the data using `colorGenoBy`. Specify custom colors i
The AMMI plot function has many options to customize the plot. It is possible to plot different principal components on n `colGeno`. Similarly for coloring environments use `colorEnvBy` and `colEnv`.
