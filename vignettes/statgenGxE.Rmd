...
This vignette describes how to perform the different types of analysis that are available in the package. The availability of functions in the package is based on the analyses described in @Malosetti2013.

The following types of analysis can be done using statgenGxE:

* [Mixed model analysis of GxE tables of means](#vcomp)
...
Before doing any analysis, we can first have a look at the contents of the data. To do this a box plot can be made. Coloring the boxes by environmental scenario will provide valuable extra information.
...
In all plots the default colors for both genotypes and trials are chosen from a predefined color palette. For genotypes the color palette is "Dark 2", for trials it is "Alphabet". See [here](https://developer.r-project.org/Blog/public/2019/11/21/a-new-palette-for-r/index.html) for an overview of these colors.    
...
If a plot has more genotypes than the number of colors specified as default colors, the default colors will be ignored and `topo.colors` will be used instead. For trials this is done in a similar way.

---

# Mixed model analysis of GxE tables of means {#vcomp}

To investigate the structure of the genotype by environment data, a mixed model can be fitted. In the statgenGxE package this can be done using the `gxeVarComp` function. 

Six different types of models can be fitted depending on the structure of the environments in the data. These models are described in the table below, together with the parameters used in `gxeVarComp` to fit the model.
...
*Note that if the data contains replicates or weights are applied (using the option `useWt`) the last term for each model will be dropped, and replaced by a homogeneous residual.*    
...
Then a model is fitted where all model terms are included as random terms. Based on the variance components in this model the percentage of variance explained by each of the model components is determined. This is printed in the model summary and plotted as standard deviations.    

### Mixed model {-}
Finally a mixed model is fitted as specified in the table above. Based on this model variance components can be extracted, heritabilities can be computed and predictions can be made. It is also possible to plot the results.

```{r geVarComp, message=FALSE} 
...
```{r vcHerit, R.options=list(digits=4)}
...
```{r predict, R.options=list(digits=4)}
...
summary(dropsFW)
```
Three types of plots can be made to investigate the output from the analysis. `plotType = "scatter"` creates three scatter plots where genotypic mean, square root of the mean squared deviation and sensitivity are plotted against each other. 
...
The model fitted in the analysis is $y_{ij} = \mu + G_i + E_j + \Sigma_{m =1}^M\gamma_{mi}\delta_{mj} + \epsilon_{ij}$, where $M$ is the number of principal components, $y_{ij}$ is the phenotypic value of genotype $i$ in environment $j$, $\mu$ is the general mean, $G_i$ is the genotypic effect, $E_j$ the environmental effect, $\gamma_{mi}$ the genotypic scores, $\delta_{mj}$ the environmental scores, and $\epsilon_{ij}$ a residual.
...
```{r geGGE, R.options=list(digits=4)}
