...
The model fitted in the analysis is 
$y_{ijk} = \mu + g_i + \Sigma_k\delta_{ik}\gamma_{jk} + \epsilon_{ijk}; \epsilon_{ijk} \sim N(0, \sigma^2_\epsilon)$.
...
A pdf report can be made as well containing a summary of the analysis.
...
The model fitted in the AMMI analysis is 
$y_{ijk} = \mu + g_i + e_j + \Sigma_k\delta_{ik}\gamma_{jk} + \epsilon_{ijk}; \epsilon_{ijk} \sim N(0, \sigma^2_\epsilon)$.
...
For the AMMI analysis a pdf report can be made using the `report` function.
...
The model fitted in the GGE analysis is 
$y_{ijk} = \mu + e_j + \Sigma_k\delta_{ik}\gamma_{jk} + \epsilon_{ijk}; \epsilon_{ijk} \sim N(0, \sigma^2_\epsilon)$.
...
The values for the Best Linear Unbiased Predictors (BLUPs) and associated standard errors for the genotypes based on the calculated mega environments, can be computed using the function `gxeTable`. This can be done using either lme4 [@Bates2015] or asreml [@Gilmour2017] as an engine for fitting the model.

```{r geMegaEnvPred, R.options=list(digits=3), eval = FALSE}
...
The models fitted are of the form $y_{ij} = \mu_{i} + \epsilon_{ij}$, where the variance of $\epsilon_{ij}$ is modeled in the following ways:
