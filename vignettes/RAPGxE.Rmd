...
For the computation of mega environments an AMMI model is fitted and then using the fitted values the environments are clustered  
...
Since the test data doesn't contain information about the year only the first method is available for this data. This is the default setting for the corresponding function.
```{r geMegaEnv}
...
As an alternative we could look at the genotypes with the lowest value per environment and try to calculate mega environments based on that. For this the parameter method in the function can be set to "min"
```{r geMegaEnvMin}
...
Shukla's stability variance is defined as the variance around the germplasm’s phenotypic mean across all environments. This provides a measure of the consistency of the genotype, without accounting for performance.
