...
test_that("varComp summary produces correct output", {
  ...
  expect_true("Sources of variation for fully random model:" %in% sumVCLm)
  expect_true(" t1 ~ (1 | trial) + (1 | genotype) " %in% sumVCLm)
  expect_true("Analysis of Variance Table for fully fixed model:" %in% sumVCLm)
  expect_true(" t1 ~ trial + genotype " %in% sumVCLm)
  ...
  expect_true("Sources of variation for fully random model:" %in% sumVCAs)
  expect_true(" t1 ~ (1 | trial) + (1 | genotype) " %in% sumVCAs)
  expect_true("Analysis of Variance Table for fully fixed model:" %in% sumVCAs)
  expect_true(" t1 ~ trial + genotype " %in% sumVCAs)
})
