...
#' @param subDesign A string specifying whether to analyse a row-column (rowcol) or resolvable row-column design (res.rowcol).
...
ST.mod.rowcol <- function(Y,
  ...
  if (engine == "asreml"){
    if (subDesign == "res.rowcol") {
      res <- ST.Varowcol(Y = Y, trait = trait, covariate = covariate, genotype = genotype,
