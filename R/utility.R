...
createReport <- function(x,
  ...
  outTex <- paste0(dirname(outfile), "/", outBase, "tex")
  ...
  knitr::opts_chunk$set(dev = "pdf",
                        fig.align = "center",
                        fig.show = "hold",
                        fig.path = paste0(dirname(outfile), "/figures/fig-"),
                        tidy = TRUE)
  knitr::knit(input = reportFile, output = outTex, quiet = TRUE)
  ## Run pdflatex twice to get a proper outputfile with correct references.
  ## Argument stdout = TRUE is needed to suppress console output.
  system2(command = Sys.which("pdflatex"),
          args = c(paste0(' -output-directory="', dirname(outfile), '"'),
                   "-interaction=nonstopmode", "-draftmode",
                   paste0(' "',  outTex, '"')), stdout = TRUE)
  system2(command = Sys.which("pdflatex"),
          args = c(paste0(' -output-directory="', dirname(outfile), '"'),
                   "-interaction=nonstopmode",
                   paste0(' "',  outTex, '"')), stdout = TRUE)
  ## Rerun knitr with chunk options set to properly process .tex.
