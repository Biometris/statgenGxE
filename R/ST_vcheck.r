...
#'                      factorNames=c("Env","Genotype","Plot", "Rep", "Row", "Column"),
#'                      traitNames="yield", env ="Env", rowSelect="DRIP05")
...
ST.vcheck <- function(data,
  ...
  indicator <- data[, trait, drop = FALSE]
  ...
  for (ii in 1:length(trait)) {
    ...
    if (nn > 0) {
      pMat0 <- data.frame(traitName = character(nn), traitValue = numeric(nn),
                          ...
                          replicate = character(nn), subBlock = character(nn),
      ...
      #pMat0$ENVNM <- data[genoRLarge, env]
      pMat0$traitValue <- data[genoRLarge, trait[ii]]
      pMat0$traitName <- rep(x = trait[ii], times = nn)
      pMat0$genotype <- data[genoRLarge, genotype]
      pMat0$residual <- stdRes[genoRLarge, ii]
      pMat0$similar <- similar[genoRLarge]
      if (!is.na(entry)) {
        pMat0$entry <- data[genoRLarge, entry]
      } else {
        pMat0$entry <- rep(NA, nn)
      }
      if (!is.na(plotNo)) {
        pMat0$plotNo <- data[genoRLarge, plotNo]
      } else {
        pMat0$plotNo <- rep(NA, nn)
      }
      if (!is.na(rep)) {
        pMat0$replicate <- data[genoRLarge, rep]
      } else {
        pMat0$replicate <- rep(NA, nn)
      }
      if (!is.na(subBlock)) {
        pMat0$subBlock <- data[genoRLarge, subBlock]
      } else {
        pMat0$subBlock <- rep(NA, nn)
      }
      if (!is.na(row)) {
        pMat0$row <- data[genoRLarge, row]
      } else {
        pMat0$row <- rep(NA, nn)
      }
      if (!is.na(col)) {
        pMat0$column <- data[genoRLarge, col]
      } else {
        pMat0$column <- rep(NA, nn)
      }
      if (!is.na(rowCoordinates)) {
        pMat0$rowPosition <- data[genoRLarge, rowCoordinates]
      } else {
        pMat0$rowPosition <- rep(NA, nn)
      }
      if (!is.na(colCoordinates)) {
        pMat0$colPosition <- data[genoRLarge, colCoordinates]
      } else {
